...
export function Photo({ src, alt, w = 400, eager }: { src: string | null; alt: string; w?: number; eager?: boolean }) {
  const url = img(src, w);
  // eslint-disable-next-line @next/next/no-img-element
  return url ? <img src={url} alt={alt} loading={eager ? "eager" : "lazy"} /> : <div className="skeleton" style={{ width: "100%", height: "100%" }} />;
}
...
export function PiscoCard({ p, children }: { p: Card; children?: React.ReactNode }) {
  ...
  return (
    <Link href={`/pisco/${p.slug}`} className={`card${p.featured ? " featured" : ""}`}>
      {children}
      <div className="photo"><Photo src={p.photo} alt={`${p.name} · ${p.producer.name}`} /></div>
